...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id)) {  // Aquí estaba el error de sintaxis
        echo json_encode(['success' => false, 'message' => 'ID de Recoleccion no proporcionado']);
        exit;
    }
    ...
    try {
        $sql = "UPDATE recoleccion SET status = ? WHERE id_recol = ?";
        $stmt = $conn_mysql->prepare($sql);
        $stmt->bind_param('si', $nuevoStatus, $id);
        $stmt->execute();
        logActivity('ACT/DEC', $Registo.' la recolección ' . $id);
