...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $sql = "UPDATE clientes SET status = ? WHERE id_cli = ?";
        $stmt = $conn_mysql->prepare($sql);
        $stmt->bind_param('si', $nuevoStatus, $id);
        $stmt->execute();

        $conn_mysql->query("UPDATE direcciones set status = '$nuevoStatus' where id_us = '$id'");

        logActivity('ACT/DEC', $Registo.' el cliente ' . $id);
