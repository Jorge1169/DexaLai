...
if ($id) {
    // Obtener datos del cliente
  $sqlCliente = "SELECT * FROM proveedores WHERE id_prov = ?";
  ...
  $sqlDireccion = "SELECT * FROM direcciones WHERE id_prov = ?";
  ...
  try {
    ...
    foreach ($ClienteData as $key => $value) {
      if ($key !== 'id_prov') {
        ...
        $sql = "UPDATE proveedores SET " . implode(', ', $setParts) . " WHERE id_prov = ?";
        $stmt = $conn_mysql->prepare($sql);
        $stmt->bind_param($types . 'i', ...$values);
        $stmt->execute();

        if ($stmt->affected_rows > 0) {
          alert("Proveedor actualizado exitosamente", 1, "V_proveedores&id=$id");
          logActivity('EDITAR', 'Edito el proveedor '. $id);
        } else {
          alert("No se realizaron cambios en el proveedor", 1, "V_proveedores&id=$id");
          logActivity('EDITAR', 'No realizo cambios en el proveedor '. $id);
        }
      } catch (mysqli_sql_exception $e) {
        alert("Error: " . $e->getMessage(), 0, "E_proveedor&id=$id");
        logActivity('EDITAR', 'Error al querer editar el proveedor '. $id);
      }
    }
    ?>
    ...
    <div class="container mt-2">
      <div class="card shadow-sm">
        <div class="card-header encabezado-col text-white d-flex justify-content-between align-items-center">
          <h5 class="mb-0">Editar Proveedor: <?= htmlspecialchars($cliente['nombre'] ?? '') ?></h5>
